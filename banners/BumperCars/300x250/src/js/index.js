...
click_through.onmouseover = function () {	
	TweenMax.to("#cta", 0.2, {scale:1.1, y:0, transformOrigin:"25% 90%", z:0.01, force3D:true, rotationZ: 0.01, transformPerspective: 400});
};
...
click_through.onmouseout = function () {
	...
	nameSpace.initAnimation = function () {
		...
		timeline.to("#copy-1", 0.8, {x:0, transformStyle: "preserve-3d", autoAlpha:1, force3D:true, rotationZ: 0.01, ease:Power1.easeOut})
			.fromTo(['#bumper-1'], 2, {x:0, ease:Power1.easeNone},{x:60}, '-=0.8')
			.fromTo(['#bumper-2'], 0.5, {x:190},{x:45, ease:Power4.easeNone}, '-=0.6')
			.to(['#bumper-1'], 0.2, {x:40, ease:Power4.easeNone}, "-=0.2")
			.to(['#bumper-2'], 0.3, {x:65, ease:Power4.easeNone}, "-=0.1")
			.to(['#bumper-1'], 0.2, {x:50, ease:Power2.easeNone}, "-=0.1")
			// .to(['#bumper-2'], 0.2, {x:60, ease:Power1.easeNone}, "-=0.1")
			.to(["#copy-1"], 0.8, {x:300, transformPerspective: 400, autoAlpha:1, force3D:true, rotationZ: 0.01, ease:Power1.easeIn}, "+=.5")
			.fromTo("#copy-2", 0.8, {x:-300}, {x:0,transformPerspective: 400,force3D:true, rotationZ: 0.01,ease:Power2.easeOut},"-=0.2")
			// .to(['#bumper-2'], 0.5, {x:40,ease:Power1.easeOut}, "-=0.4")
			.to(['#bumper-2'], 0.5, {scale:0.85,transformOrigin:"50% 50%",ease:Power1.easeOut})
			.to(['#bumper-1'], 0.4, {x:50, transformOrigin:"55% 50%",scale:1.03, ease:Power1.easeOut}, "-=0.5")
		
			.to(['#bumper-2'], 1,  {x:-250, ease:Power1.easeOut})
			
			.to(["#copy-2"], 0.8, {x:width, transformPerspective: 300, autoAlpha:1, force3D:true, rotationZ: 0.01, ease:Power1.easeOut}, "+=0.6")
			...
			.fromTo("#copy-4", 0.8, {x:-300, autoAlpha:1}, {x:0, autoAlpha:1, force3D:true, rotationZ: 0.01,ease:Power2.easeOut},"-=.4")
	
			.to("#icon", 0.4, {autoAlpha:1}, '-=0.2')
			.fromTo(['#check-3'], 1, {x:-160, y:-120, scale:0.7, rotation:100, transformOrigin:"50% 50%", ease:Power2.easeOut},{scale:1, x:0, y:-30, rotation:0, ease:Power2.easeOut} ,'-=0.3')
			.to("#pen", 0.4, {autoAlpha:1})
			// .to("#icon", 0.8, {autoAlpha:1, scale:0.75, x:-30, y:-10})	
			.to(["#copy-5"], 0.4, { autoAlpha:1, transformPerspective: 300, force3D:true, rotationZ: 0.01, ease:Power2.easeIn})	
