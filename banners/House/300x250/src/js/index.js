	...
	nameSpace.init = function () {
		...
		height = 250;
			...
			//
				 ...
				 TweenMax.set(["#moon"], {scale:1.1, autoAlpha:0});
				 TweenMax.set(["#logo", "#cta", "#code", "#copy-2","#copy-3"], {autoAlpha:0});
				 TweenMax.set('#shape', {scale:1, autoAlpha:0});
				 TweenMax.set(['#bg-color'] , { autoAlpha:0, y:0});
				 TweenMax.set("#outline", {scale:6, x:200, y:-100});
...
click_through.onmouseover = function () {	
	TweenMax.to("#cta", 0.2, {scale:1.1, y:0, transformOrigin:"75% 60%", z:0.01, force3D:true, rotationZ: 0.01, transformPerspective: 400});
};
...
click_through.onmouseout = function () {
	...
	nameSpace.initAnimation = function () {
		...
		timeline.to("#copy-1", 0.5, {autoAlpha:1},"+=1")
				 .to("#house", 1.5, {y:-20, scale:.92, x:15})
				 .to("#grass", 1.5, {y:-9}, "-=1.5")
				.to("#copy-2", 0.8, {autoAlpha:1}, "+=.3")
				 ...
				 .to("#sky-holder", 1.5, {scale:0.68, x:"-102", y:"45"})
				 .to("#outline",1.5, {x:0, y:0, scale:1, onComplete:stopSmoke}, "-=1.5")
				 .set("#copy-3", {x:300})
				 .to("#copy-3", 0.5, {autoAlpha:1})
				 .to(".walls", 1.5, {fill:"#cb7a17"})
				 .to("#sky", 1.5, {backgroundColor:"#0e4c86"}, "-=1.5")
				 .to("#moon",0.8, {y:26,autoAlpha:1})
	...
	function animateSmoke() {
		...
		var ts = new TimelineMax();
			ts.fromTo("#s1", 2, {y:0, autoAlpha:1}, {y:-50, autoAlpha:0})
			.fromTo("#s2", 2, {y:0, autoAlpha:1}, {y:-50, autoAlpha:0}, "-=2")
			.fromTo("#s3", 2, {y:0, autoAlpha:1}, {y:-50, autoAlpha:0,onComplete:animateSmoke}, "-=2");
		}
	}
	...
	function startBgImg() {
		...
		//TweenMax.fromTo("#shape", 14.5,{ autoAlpha:0, y:500, scale:4, ease:Cubic.easeInOut}, { transformPerspective: 300, autoAlpha:1, force3D:true, rotationZ: 0.01, y:0, scale:1,ease:Cubic.easeInOut});
	}
